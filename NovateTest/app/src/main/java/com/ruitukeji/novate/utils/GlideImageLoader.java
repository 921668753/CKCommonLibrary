...
//                    .into(imageView);
            GlideApp.with(activity)
                    .asBitmap()//配置上下文
                    .load(path)//设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
//                    .error(com.common.cklibrary.R.mipmap.default_image)           //设置错误图片
//                    //  .placeholder(R.mipmap.load)     //设置占位图片
//                    .fallback(com.common.cklibrary.R.mipmap.default_image)//当url为空时，显示图片
                    .diskCacheStrategy(DiskCacheStrategy.ALL)//缓存全尺寸
                    .centerInside()
                    //  .transition(withCrossFade().crossFade())//应用在淡入淡出
                    //  .skipMemoryCache(true)//设置跳过内存缓存
...
//                    .error(com.common.cklibrary.R.mipmap.avatar)
//                    .fallback(com.common.cklibrary.R.mipmap.avatar)
            ...
            GlideApp.with(context)
                    .load(url)
                    //  .placeholder(R.mipmap.loading)
                    //    .error(com.common.cklibrary.R.mipmap.placeholderfigure)
    ...
    public static void glideLoader(Context context, Object url, ImageView imageView, int radius, int bottom, int defaultimage) {
        GlideApp.with(context)
                .load(url)
                //  .placeholder(R.mipmap.loading)
//                    .error(com.common.cklibrary.R.mipmap.default_image)
